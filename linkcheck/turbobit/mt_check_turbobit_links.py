...
log_format = '%(asctime)s [%(levelname)-5s] (%(threadName)-10s) %(message)s'
...
if __name__ == '__main__':
    ...
    logging.getLogger('check_links').addHandler(debugLogger)
    ...
    #pu.log2file('log/fetcher.log', logging.INFO)
    ...
    links_qs = Link.objects.filter(
            Q(domain='turbobit.net')|Q(domain='turbobit.name')
        ).exclude(is_valid=1)
    ...
    count = 0
    for i in iterator:
        if 1: #is_good_url(i.url):
            q.put(i)
            debug('put(): %s' % i)
            count += 1
            #if count > num_links:
            #    debug('num_links exceeded: %d' % count)
            #    break
        else:
            debug('SKIPPED #%d url: %s' % (i.pk, i.url))
